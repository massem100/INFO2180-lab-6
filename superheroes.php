...
$data  = ucwords(filter_var(htmlspecialchars(trim($_GET["query"])), FILTER_SANITIZE_STRING, FILTER_FLAG_NO_ENCODE_QUOTES));
if ($data === '') : ?>
    <h2 class=>Result</h2>
    <hr>
    <ul>
...
<?php else :   ?>
    <?php $finder= 1; ?>
    <?php foreach ($superheroes as $superhero) : ?>
        <?php if ($superhero['name'] === $data || $superhero['alias'] === $data) : ?>
            <h2 >Result</h2>
            <hr >
        ...
        <?php else :  ?>
            ...
            <?php if ($finder === 11) : ?>
                <h2>Result</h2>
                <hr>
                <h4 class ="redError">SUPERHERO NOT FOUND</h3>
                <?php endif; ?>
